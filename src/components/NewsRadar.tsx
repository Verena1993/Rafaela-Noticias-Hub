...
import { MEDIA_PRIORITY, EDITORIAL_FILTERS, ALERT_CRITICAL_KEYWORDS, FILTER_KEYWORDS } from '../utils/mediaConfig';
import { getSourceRegion } from '../config/newsSourceRegions';

type RadarMainTab = 'media' | 'feed' | 'opportunities' | 'topics' | 'local' | 'provincial' | 'national' | 'trends' | 'rafaela_talks' | 'diagnostics';
...
export const NewsRadar: React.FC = () => {
  const { newsRadarItems, updateNewsRadarItem, addProposal, fetchLiveRadarNews, loadingRadar, radarError, lastRadarUpdate, coverages, proposals, rssDiagnostics } = useHub();
  
  const [mainTab, setMainTab] = useState<RadarMainTab>('media');
  ...
  const localItems = newItems.filter(i => getSourceRegion(i.source) === 'local');
  const provincialItems = newItems.filter(i => getSourceRegion(i.source) === 'provincial');
  const nationalItems = newItems.filter(i => getSourceRegion(i.source) === 'national');
  ...
  // --- NEW MEDIA VIEW LOGIC ---
  const alertItems = React.useMemo(() => {
    return newItems.filter(item => {
      const title = item.title.toLowerCase();
      return ALERT_CRITICAL_KEYWORDS.some(kw => title.includes(kw));
    }).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [newItems]);

  const filteredForMedia = React.useMemo(() => {
    return newItems.filter(item => {
      if (mediaFilter === 'Todas') return true;
      const keywords = FILTER_KEYWORDS[mediaFilter];
      if (!keywords) return true;
      const text = `${item.title} ${item.summary} ${item.category}`.toLowerCase();
      return keywords.some(kw => text.includes(kw));
    });
  }, [newItems, mediaFilter]);

  const mediaGroups = React.useMemo(() => {
    const itemsByMedia: Record<string, NewsRadarItem[]> = {};
    filteredForMedia.forEach(item => {
      if (!itemsByMedia[item.source]) itemsByMedia[item.source] = [];
      itemsByMedia[item.source].push(item);
    });

    return Object.keys(itemsByMedia).sort((a, b) => {
      // 1. Sort by Region (local -> provincial -> national -> other)
      const regionA = getSourceRegion(a);
      const regionB = getSourceRegion(b);
      const regionPriority = { local: 1, provincial: 2, national: 3, other: 4 };
      if (regionPriority[regionA] !== regionPriority[regionB]) {
        return regionPriority[regionA] - regionPriority[regionB];
      }

      // 2. Sort by Media Priority inside region
      let idxA = MEDIA_PRIORITY.indexOf(a);
      let idxB = MEDIA_PRIORITY.indexOf(b);
      if (idxA === -1) idxA = 999;
      if (idxB === -1) idxB = 999;
      ...
      // Sort items inside media: Relevancia (Alertas primero), Impacto Local, luego Reciente
      const sortedItems = itemsByMedia[source].sort((a, b) => {
        const aIsAlert = isAlert(a.title) ? 1 : 0;
        const bIsAlert = isAlert(b.title) ? 1 : 0;
        if (aIsAlert !== bIsAlert) return bIsAlert - aIsAlert;
        
        const aLocal = getSourceRegion(a.source) === 'local' ? 1 : 0;
        const bLocal = getSourceRegion(b.source) === 'local' ? 1 : 0;
        if (aLocal !== bLocal) return bLocal - aLocal;

        return new Date(b.date).getTime() - new Date(a.date).getTime();
      }).slice(0, 3);
      
      return { source, items: sortedItems };
    });
  }, [filteredForMedia]);

  const quickOpportunities = React.useMemo(() => {
    // Quick opportunities independent of topic grouping:
    // Only 1 source (meaning it doesn't appear multiple times in newItems)
    // Local or provincial
    const sourceCount: Record<string, Set<string>> = {};
    newItems.forEach(i => {
      const normTitle = i.title.toLowerCase().substring(0, 20);
      if (!sourceCount[normTitle]) sourceCount[normTitle] = new Set();
      sourceCount[normTitle].add(i.source);
    });

    return newItems.filter(i => {
      const normTitle = i.title.toLowerCase().substring(0, 20);
      const isSingleSource = sourceCount[normTitle].size === 1;
      const isLocal = i.category === 'local' || i.category === 'provincial';
      const isRecent = (new Date().getTime() - new Date(i.date).getTime()) < 12 * 3600000;
      return isSingleSource && isLocal && isRecent;
    }).slice(0, 5); // top 5 quick opps
  }, [newItems]);
    ...
    <div>
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginTop: '0.25rem' }}>
            ...
            {lastRadarUpdate && (
              <span style={{ marginLeft: '0.5rem', opacity: 0.8 }}>
                Última actualización: {new Date(lastRadarUpdate).toLocaleTimeString()}
              </span>
            )}
          </p>
          {radarError && (
      ...
      {newsRadarItems.length === 0 ? (
        <div style={{ padding: '4rem', textAlign: 'center', background: 'var(--bg-secondary)', borderRadius: 'var(--radius-md)' }}>
          ...
          <h3 style={{ fontSize: '1.2rem', color: 'var(--text-secondary)', fontWeight: 600 }}>No hay noticias disponibles actualmente.</h3>
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', borderBottom: '1px solid var(--border-color)', marginBottom: '1.5rem', overflowX: 'auto', paddingBottom: '0.1rem' }}>
            <TabButton tab="media" icon={Newspaper} label="Medios" color="#3b82f6" />
            <TabButton tab="feed" icon={Activity} label="Feed Global" />
          ...
          {mainTab === 'media' && (
            <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
              ...
              {/* LEFT: MEDIA FEED */}
              <div style={{ flex: '1 1 60%', minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.5rem' }}>
                  {EDITORIAL_FILTERS.map(f => (
                    <button 
                      key={f}
                      className={`btn ${mediaFilter === f ? 'btn-primary' : 'btn-secondary'}`} 
                      style={{ fontSize: '0.75rem', padding: '0.3rem 0.6rem', borderRadius: 'var(--radius-full)' }}
                ...
                {mediaGroups.length === 0 ? (
                  ...
                  mediaGroups.map(group => (
                    <div key={group.source} style={{ marginBottom: '1.5rem' }}>
                      <h3 style={{ fontSize: '1rem', fontWeight: 800, textTransform: 'uppercase', borderBottom: '2px solid var(--primary)', paddingBottom: '0.3rem', marginBottom: '0.75rem', color: 'var(--text-primary)' }}>
                        {group.source}
                      </h3>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        {group.items.map(item => (
                          <div key={`mview_${item.id}`} style={{ padding: '0.75rem', background: 'var(--bg-secondary)', borderRadius: 'var(--radius-md)', borderLeft: `3px solid ${CATEGORY_CONFIG[item.category]?.color || '#999'}` }}>
                            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginBottom: '0.3rem', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                              <span style={{ fontWeight: 600, color: CATEGORY_CONFIG[item.category]?.color }}>{CATEGORY_CONFIG[item.category]?.label || item.category}</span>
                              <span>•</span>
                              <Clock size={10} /> {formatDate(item.date)}
                            </div>
                            <h4 style={{ fontSize: '0.9rem', fontWeight: 700, margin: '0 0 0.4rem 0' }}>{item.title}</h4>
                            {item.url && (
                              <a href={item.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.75rem', color: 'var(--primary)', display: 'inline-flex', alignItems: 'center', gap: '0.2rem', textDecoration: 'none' }}>
                                Leer original <ExternalLink size={10} />
                              </a>
                            )}
                          </div>
                        ))}
              ...
              {/* RIGHT: SIDEBAR (ALERTAS & OPORTUNIDADES) */}
              <div style={{ flex: '0 0 35%', minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                
                {/* ALERTAS */}
                <div style={{ background: 'var(--bg-secondary)', borderRadius: 'var(--radius-md)', border: '1px solid #ef4444', overflow: 'hidden' }}>
                  <div style={{ background: '#ef4444', color: 'white', padding: '0.5rem 1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: 800 }}>
                    <AlertTriangle size={16} /> ALERTAS
                  </div>
                  <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {alertItems.length === 0 ? (
                      <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Sin alertas críticas recientes.</span>
                    ) : (
                      alertItems.slice(0, 5).map(alert => (
                        <div key={`al_${alert.id}`} style={{ borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>
                          <span style={{ fontSize: '0.65rem', fontWeight: 700, color: '#ef4444', backgroundColor: 'rgba(239,68,68,0.1)', padding: '0.1rem 0.3rem', borderRadius: '2px', display: 'inline-block', marginBottom: '0.2rem' }}>Prioridad Crítica</span>
                          <h5 style={{ margin: '0 0 0.3rem 0', fontSize: '0.85rem', fontWeight: 700 }}>{alert.title}</h5>
                ...
                <div style={{ background: 'var(--bg-secondary)', borderRadius: 'var(--radius-md)', border: '1px solid #8b5cf6', overflow: 'hidden' }}>
                  <div style={{ background: '#8b5cf6', color: 'white', padding: '0.5rem 1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: 800 }}>
                    <Target size={16} /> OPORTUNIDADES RÁPIDAS
                  </div>
                  <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {quickOpportunities.length === 0 ? (
                      <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>No hay oportunidades detectadas.</span>
                    ) : (
                      quickOpportunities.map(opp => (
                        <div key={`qo_${opp.id}`} style={{ borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>
                          <span style={{ fontSize: '0.65rem', fontWeight: 700, color: '#8b5cf6', backgroundColor: 'rgba(139,92,246,0.1)', padding: '0.1rem 0.3rem', borderRadius: '2px', display: 'inline-block', marginBottom: '0.2rem' }}>
                            ⭐ {opp.category === 'local' ? 'Posible Exclusiva Local' : 'Posible Seguimiento'}
                          </span>
                          <h5 style={{ margin: '0 0 0.3rem 0', fontSize: '0.85rem', fontWeight: 700 }}>{opp.title}</h5>
                          <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', display: 'flex', justifyContent: 'space-between' }}>
                            <span>Única fuente: {opp.source}</span>
